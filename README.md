...
### Shared provider keys on GitHub Pages

In the publishing repository, open **Settings → Secrets and variables → Actions → New repository secret** and add `ESRI_API_KEY` and/or `MAPTILER_API_KEY`. Use a dedicated basemap-only key restricted to the site's referrer. The Pages workflow generates `map-provider-config.js` from these secrets before uploading the site. Selecting a configured provider then uses its key automatically. Without the corresponding secret, the manual key field remains available.
